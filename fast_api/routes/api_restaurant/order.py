...
from fast_api.core.database.model.restaurant.financial import MovimentacaoCaixa, Caixa, StatusCaixa
...
from fast_api.core.database.model.restaurant.orders import Pedido, ItemPedido, StatusPedido,Base
...
class PedidoCreate(BaseModel):
    cliente_id: int | None = None  # Pode ser nulo se for venda rápida no balcão
    observacao: str | None = None

    itens: List[ItemPedidoCreate]

    tipo_entrega: str  # Ex: "DELIVERY" ou "RETIRADA"
    endereco_entrega: Optional[str] = None  # A "foto" do endereço para este pedido
    forma_pagamento: Optional[str] = None  # Ex: "PIX", "DINHEIRO", "CARTAO"
...
class ItemPedidoResponse(BaseModel):
    ...
    class Config:
        from_attributes = True  # Se for Pydantic V2 (Ou orm_mode = True no V1)


# 2. Depois criamos o schema do PEDIDO e colocamos a lista de itens nele
class PedidoResponse(BaseModel):
    ...
    # A MÁGICA FINAL: O FastAPI agora sabe que deve exibir uma lista
...
def obter_pedido(
    ...
    # Dica: Se você configurou 'itens = relationship("ItemPedido")' no model Pedido,
    # os itens virão automaticamente junto com a resposta!
