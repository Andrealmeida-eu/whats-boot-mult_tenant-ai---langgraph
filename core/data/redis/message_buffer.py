...
async def buffer_message(tenant_id: str, chat_id: str, message: str, name_cliente: str):
    ...
    log(f"Mensagem adicionada ao buffer de {tenant_id}:{chat_id}: {message} ---- {name_cliente}")
    ...
    if old and not old.done():
        old.cancel()
        log(f"Debounce resetado para {tenant_id}:{chat_id} ---- {name_cliente}")
    ...
    t = loop.create_task(handle_debounce(tenant_id, chat_id, token, name_cliente))
    t.add_done_callback(lambda tt: _task_done(tt, tenant_id, chat_id))
    debounce_tasks[key] = t


async def handle_debounce(tenant_id: str, chat_id: str, token: int, name_cliente: str):
    key = (tenant_id, chat_id)
    buffer_key = f"{tenant_id}:{chat_id}{settings.BUFFER_KEY_SUFIX}"

    print(f"[DEBOUNCE] ENTROU {tenant_id}:{chat_id} token={token} ---- {name_cliente}", flush=True)
    ...
    with contextmanager(get_db)() as db:
        try:
            ...
            log(f"[DEBOUNCE] Iniciando debounce para {tenant_id}:{chat_id} token={token} ---- {name_cliente}")
            await asyncio.sleep(float(settings.DEBOUNCE_SECONDS))

            # se chegou mensagem nova, não envia
            if debounce_tokens.get(key) != token:
                log(f"[DEBOUNCE] Task antiga ignorada {tenant_id}:{chat_id} token={token} ---- {name_cliente}")
                return
            ...
            log(f"status loja {status_loja} ---- {name_cliente}")
            ...
            if "main_graph" not in graphs_cache:
                log("Compilando o grafo mestre pela primeira vez...")
                graphs_cache["main_graph"] = await build_graph()
            
            graph = graphs_cache["main_graph"]
            
            key_client = f"{tenant_id}:{chat_id}"
            
            thread_id_valid = str(uuid.uuid5(uuid.NAMESPACE_DNS, key_client))
            
            config = {
                "configurable": {
                    "thread_id": thread_id_valid
                }
            }
            log(f"config: {config}")
            ...
            current_step = result_graph["current_step"]
            tipo_entrega = result_graph["tipo_entrega"]
            log(f"voltei do invoke: {ai_response}")
            log(f"tipo_entrega  : {tipo_entrega}")
            log(f" : {current_step}")
