...
from core.providers.evolution import EvolutionProvider

# -> importa a implementação concreta do provedor Meta Cloud
from core.providers.meta import MetaCloudProvider
...
def get_provider(tenant):
    ...
    if tenant.whatsapp_provider == "evolution":
        return EvolutionProvider(
            instance_url=cfg.get("instance_url"),              # -> URL base da Evolution API
            instance_name=cfg.get("instance_name"),  # -> nome da instância
            global_apikey=cfg.get("global_apikey")               # -> chave de autenticação
        )
