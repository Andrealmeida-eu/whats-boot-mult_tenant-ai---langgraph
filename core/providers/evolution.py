...
class EvolutionProvider(WhatsAppProvider):
    ...
    def __init__(self, instance_url, instance_name, global_apikey):
    ...
    def send_text(self, to: str, text: str):
        ...
        url = f"{self.instance_url}/message/sendText/{self.instance_name}"
        
        # -> headers obrigatórios para autenticação e tipo de conteúdo
        headers = {
            "apikey": self.api_key,                # -> autenticação na Evolution
